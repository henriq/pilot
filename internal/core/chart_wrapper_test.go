...
func TestChartWrapper_Generate_Success(t *testing.T) {
	...
	require.NoError(t, err)
	expectedAbsPath := filepath.Join(home, ".dx", "test-context", "wrapper-charts", "test-release")
	...
	assert.Contains(t, string(fs.writtenFiles[chartYamlPath]), "dx.wrapped-chart: \"original-chart\"")
...
func TestChartWrapper_Generate_SanitizesContextName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			expectedPath := filepath.Join(home, ".dx", tt.expectedContext, "wrapper-charts", "valid-release")
...
func TestGenerateChartYaml_WithAnnotations(t *testing.T) {
	...
	assert.Contains(t, yaml, "dx.wrapped-chart: \"original\"")
	assert.Contains(t, yaml, "dx.wrapped-path: \"/path/to/chart\"")
}
...
func TestGenerateChartYaml_EscapesSpecialChars(t *testing.T) {
	...
	assert.Contains(t, yaml, `dx.wrapped-chart: "chart-with-\"quotes\""`)
	assert.Contains(t, yaml, `dx.wrapped-path: "/path/with\nnewline"`)
	// Should NOT contain unescaped newline that would break YAML structure
	assert.NotContains(t, yaml, "dx.wrapped-path: \"/path/with\n")
}
...
func TestExpandTildePath(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    filepath.Join("~", ".dx", "context"),
			expected: filepath.Join(home, ".dx", "context"),
		},
		{
